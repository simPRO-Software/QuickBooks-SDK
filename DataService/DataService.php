...
class DataService
{
	...
	public function Update($entity)
	{
		...
		if ($this->serviceContext->getSimproDebug()) {
			Console::Log("QB Data Service :: Called Method Update.", TraceLevel::Info);
		}
		// Builds resource Uri
		// Handle some special cases
		if ((strtolower('preferences') == strtolower($urlResource)) &&
				(IntuitServicesType::QBO == $this->serviceContext->serviceType)) { // URL format for *QBO* prefs request is different than URL format for *QBD* prefs request
			...
			$urlResource = 'companyInfo'; // URL format for *QBD* companyinfo request is different than URL format for *QBO* companyinfo request
		...
		if ($this->serviceContext->getSimproDebug()) {
			Console::Log("QB Data Service :: URI.", $uri);
		}
		...
		if ($this->serviceContext->getSimproDebug()) {
			Console::Log("QB Data Service :: requestParameters.", $requestParameters);
		}
		$restRequestHandler = new SyncRestHandler($this->serviceContext);
		// gets response
		$result = $restRequestHandler->GetResponse($requestParameters, $httpsPostBody, NULL);
		if (!empty($result['error']) || empty($result['array'])) {
			if (empty($result['error']) && empty($result['array'])) {
				return null;
			}
			return $result;
		}
		list($responseCode, $responseBody) = $result['array'];
	 ...
	 */
	public function FindById($entity, $debug = true)
	{
		if ($this->serviceContext->getSimproDebug()) {
			Console::Log("QB Data Service :: Called Method FindById.", TraceLevel::Info);
		}
		$httpsPostBody = XmlObjectSerializer::getPostXmlFromArbitraryEntity($entity, $urlResource);
		// Validate parameter
		if ($entity && (strtolower('preferences') == strtolower($urlResource))) {
			...
			if ($this->serviceContext->getSimproDebug()) {
				Console::Log("QB Data Service :: Argument Null Exception", TraceLevel::Error);
			}
			return NULL;
		}
		$entityId = $entity->Id;
		...
		$restRequestHandler = new SyncRestHandler($this->serviceContext);
		$result = $restRequestHandler->GetResponse($requestParameters, NULL, NULL);
		if (!empty($result['error']) || empty($result['array'])) {
			if (empty($result['error']) && empty($result['array'])) {
				return null;
			}
	...
	public function Add($entity)
	{
		...
		$httpsPostBody = XmlObjectSerializer::getPostXmlFromArbitraryEntity($entity, $urlResource);
		Console::Log('$httpsPostBody', $httpsPostBody);
	...
	public function Upload($imgBits, $fileName, $mimeType, $objAttachable)
	{
		...
		// gets response
		$result = $restRequestHandler->GetResponse($requestParameters, $dataMultipart, NULL);
		if (!empty($result['error']) || empty($result['array'])) {
			if (empty($result['error']) && empty($result['array'])) {
				return null;
			}
	...
	{
		if ($this->serviceContext->getSimproDebug()) {
			Console::Log("QB Data Service :: Called Method Query", TraceLevel::Info);
		}
		...
		$result = $restRequestHandler->GetResponse($requestParameters, $httpsPostBody, NULL);
		if ($this->serviceContext->getSimproDebug()) {
			Console::Log("QB Data Service :: result", $result);
		}
		if (!empty($result['error']) || empty($result['array'])) {
			if (empty($result['error']) && empty($result['array'])) {
				return null;
		...
		list($responseCode, $responseBody) = $result['array'];
		if ($this->serviceContext->getSimproDebug()) {
			Console::Log("QB Data Service :: responseCode, responseBody", $responseCode, $responseBody);
		}
		...
		try {
			...
			} else if ($responseXmlObj && $responseXmlObj->Fault) {
				return $responseBody; // returns array as there could be multiple
			}
		} catch (Exception $e) {
			Console::Log('$e', $e);
			return NULL;
	...
	public function FindAll($entityName, $pageNumber = 0, $pageSize = 500)
	{
		...
		Console::Log('$httpsPostBody', $httpsPostBody);
		$requestParameters = new RequestParameters($httpsUri, 'POST', $httpsContentType, NULL);
		Console::Log('$httpsUri', $httpsUri, $httpsContentType);
		$restRequestHandler = new SyncRestHandler($this->serviceContext);
		Console::Log('$restRequestHandler', $restRequestHandler);
		$result = $restRequestHandler->GetResponse($requestParameters, $httpsPostBody, NULL);
		if (!empty($result['error']) || empty($result['array'])) {
			if (empty($result['error']) && empty($result['array'])) {
				return null;
		...
		list($responseCode, $responseBody) = $result['array'];
		Console::Log('$responseCode, $responseBody', $responseCode, $responseBody);
		$parsedResponseBody = NULL;
		try {
			$responseXmlObj = simplexml_load_string($responseBody);
			Console::Log('$responseXmlObj', $responseXmlObj);
			if ($responseXmlObj && $responseXmlObj->QueryResponse) {
				$parsedResponseBody = $this->responseSerializer->Deserialize($responseXmlObj->QueryResponse->asXML(), FALSE);
			}
		} catch (Exception $e) {
	...
	/**
	 * @todo Returns the attributes of a xml object when trying to do: "SELECT COUNT(*) FROM CUSTOMER"
	 * @param type $QBQuery
	 */
	public function getTotalCount($QBQuery)
	{
